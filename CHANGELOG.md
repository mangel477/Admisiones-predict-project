...
- Un pipeline de inferencia ejecutable por sí solo, `src/pipelines/inference_pipeline/inference_pipeline.py`, que carga el modelo entrenado, lee un archivo con candidatos nuevos, los lleva a la forma con la que el modelo fue entrenado y guarda cada candidato junto a su probabilidad estimada de admisión. Reconoce los nombres de columna sin distinguir mayúsculas ni espacios, acepta la variable de investigación escrita de varias formas —sí, no, 1, 0, true, false— y acota las probabilidades al rango válido, porque una regresión lineal puede devolver valores por fuera para candidatos en los extremos.
- El pipeline de inferencia rechaza la ejecución completa, sin escribir nada, cuando la variable de investigación trae un valor que no puede interpretarse. Hasta ahora ese caso era invisible: el modelo convertía cualquier valor desconocido en la respuesta mayoritaria, de modo que un candidato que respondía "no" quedaba puntuado como si hubiera respondido "sí", con casi dos puntos porcentuales de diferencia y sin ninguna señal de que eso hubiera ocurrido.
...
- Un pipeline de entrenamiento ejecutable por sí solo, `src/pipelines/training_pipeline/train_pipeline.py`, que lee la tabla de features, separa un 20 % de los registros para prueba estratificando por quintiles de la probabilidad de admisión, entrena el modelo elegido en los notebooks, lo evalúa sobre las dos partes de la separación y guarda el modelo junto con un informe de métricas en formato JSON. Reproduce de forma exacta el modelo que hasta ahora solo podía obtenerse ejecutando un notebook a mano. Admite rutas y nivel de detalle por línea de comandos.
